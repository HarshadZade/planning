...
def graderMain(executablePath, gradingCSV):
    problems = [
        [
            "./map1.txt",
            "1.570796,0.785398,1.570796,0.785398,1.570796",
    ...
    for aPlanner in [0, 1, 2, 3]:
        ...
        for i, data in enumerate(problems):
            ...
            commandVerify = "./verifier.out {} {} {} {} {}".format(
                inputMap, numDOFs, startPos, goalPos, outputSolutionFile
            )
            print("EXECUTING: " + str(commandVerify))
            try:
                start = timer()
                subprocess.run(
                    commandPlan.split(" "), check=True
                )  # True if want to see failure errors
                timespent = timer() - start
                returncode = subprocess.run(
                    commandVerify.split(" "), check=False
                ).returncode
                if returncode != 0:
                ...
                with open(outputSolutionFile) as f:
                    ...
                    scores.append(
                        [aPlanner, inputMap, i, numSteps, cost, timespent, success]
                    )

                ### Visualize their results
                commandViz = "python visualizer.py grader_out/tmp.txt --gifFilepath=grader_out/grader_{}{}.gif".format(
                    plannerList[aPlanner], i
                )
                commandViz += " --incPrev=1"
                subprocess.run(
                    commandViz.split(" "), check=True
                )  # True if want to see failure errors
