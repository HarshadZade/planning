...
def createSingleFrame(i, mapData, allPoses, includePrevious):
    ...
    artists.append(
        plt.plot(xs[-1], ys[-1], "ro", markersize=5, label="End Effector Position")
    )
...
def viz():
    ...
    parser.add_argument(
        "fileWithSolution",
        help="filepath with solution",
        type=str,
        default="output.txt",
    )
    parser.add_argument(
        "--gifFilepath",
        help="filepath for gif",
        type=str,
        default="output.gif",
        required=False,
    )
    parser.add_argument(
        "--fps", help="frames per second", type=int, default=4, required=False
    )
    parser.add_argument(
        "--incPrev",
        help="include previous poses (1), else don't (0). Note this slows gif creation speed",
        type=int,
        default=0,
        required=False,
    )
    ...
    ani = FuncAnimation(
        ...
        repeat=False,
        frames=numFrames,
        fargs=(mapData, solution, args.incPrev),
    )
