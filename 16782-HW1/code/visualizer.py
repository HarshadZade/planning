...
if __name__ == "__main__":
    ...
    x_size, y_size, collision_threshold, robotX, robotY, target_trajectory, costmap = parse_mapfile(
        sys.argv[1]
    )
    ...
    def update(frame):
        line1.set_data(
            [p["x"] for p in robot_trajectory[: frame + 1]],
            [p["y"] for p in robot_trajectory[: frame + 1]],
        )

        t = robot_trajectory[frame + 1]["t"]
        line2.set_data(
            [p["x"] for p in target_trajectory[:t]],
            [p["y"] for p in target_trajectory[:t]],
        )

        plt.pause(
            (robot_trajectory[frame + 1]["t"] - robot_trajectory[frame]["t"]) / SPEEDUP
        )
